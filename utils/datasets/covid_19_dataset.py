...
class Covid19Dataset(Dataset):
    def __init__(self, tokenizer, target_name=None, text_with_target=True, if_split_hash_tag=True, in_target=None, zero_shot=None, train_data=None, valid_data=None, test_data=None, debug_mode=False):
        ...
        logging.info(f'Load Covid 19 Dataset')
        ...
        if in_target:
            if train_data:
                self.data_path = f'{Covid19Config.in_target_data_dir}/{target_name}/train.csv'
            elif valid_data:
                self.data_path = f'{Covid19Config.in_target_data_dir}/{target_name}/valid.csv'
            elif test_data:
                self.data_path = f'{Covid19Config.in_target_data_dir}/{target_name}/test.csv'
        elif zero_shot:
            if train_data:
                self.data_path = f'{Covid19Config.zero_shot_data_dir}/{target_name}/train.csv'
            elif valid_data:
                self.data_path = f'{Covid19Config.zero_shot_data_dir}/{target_name}/valid.csv'
            elif test_data:
                self.data_path = f'{Covid19Config.zero_shot_data_dir}/{target_name}/test.csv'
