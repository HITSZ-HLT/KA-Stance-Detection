...
class Covid19KASDDataset(Dataset):
    def __init__(self, tokenizer, target_name=None, text_with_target=True, if_split_hash_tag=True, in_target=None, zero_shot=None, train_data=None, valid_data=None, test_data=None, debug_mode=False):
        super(Covid19KASDDataset, self).__init__()
        assert train_data or valid_data or test_data
        logging.info(f'Load Covid 19 Restated Cat Knowledge Dataset')
        ...
        if in_target:
            if train_data:
                self.data_path = f'{Covid19Config.in_target_kasd_data_dir}/{target_name}/train.csv'
            elif valid_data:
                self.data_path = f'{Covid19Config.in_target_kasd_data_dir}/{target_name}/valid.csv'
            elif test_data:
                self.data_path = f'{Covid19Config.in_target_kasd_data_dir}/{target_name}/test.csv'
        elif zero_shot:
            if train_data:
                self.data_path = f'{Covid19Config.zero_shot_kasd_data_dir}/{target_name}/train.csv'
            elif valid_data:
                self.data_path = f'{Covid19Config.zero_shot_kasd_data_dir}/{target_name}/valid.csv'
            elif test_data:
                self.data_path = f'{Covid19Config.zero_shot_kasd_data_dir}/{target_name}/test.csv'
    ...
    def read_data(self, path, debug_mode=False):
        ...
        labels = []
        label_num = {}
        for label_name in Covid19Config.label2idx.keys():
        ...
        for data in all_datas:
            ...
            if Covid19Config.apply_cleaning:
