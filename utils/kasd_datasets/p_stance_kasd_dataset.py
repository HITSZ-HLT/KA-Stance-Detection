...
class PStanceKASDDataset(Dataset):
    def __init__(self, tokenizer, target_name=None, text_with_target=True, if_split_hash_tag=True, in_target=None, zero_shot=None, train_data=None, valid_data=None, test_data=None, debug_mode=False):
        super(PStanceKASDDataset, self).__init__()
        assert train_data or valid_data or test_data
        assert in_target or zero_shot
        logging.info(f'Load P-Stance KASD Dataset')
        ...
        if in_target:
            if train_data:
                self.data_path = f'{PStanceConfig.in_target_kasd_data_dir}/{target_name}/train.csv'
            elif valid_data:
                self.data_path = f'{PStanceConfig.in_target_kasd_data_dir}/{target_name}/valid.csv'
            elif test_data:
                self.data_path = f'{PStanceConfig.in_target_kasd_data_dir}/{target_name}/test.csv'
        elif zero_shot:
            if train_data:
                self.data_path = f'{PStanceConfig.zero_shot_kasd_data_dir}/{target_name}/train.csv'
            elif valid_data:
                self.data_path = f'{PStanceConfig.zero_shot_kasd_data_dir}/{target_name}/valid.csv'
            elif test_data:
                self.data_path = f'{PStanceConfig.zero_shot_kasd_data_dir}/{target_name}/test.csv'
